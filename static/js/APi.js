(function ($) {
  ...
  function fetchData() {
    fetch(API)
      .then((res) => res.json())
      .then((data) => {
        // Get the last entry (most recent feed)
        let lastEntry = data.feeds[data.feeds.length - 1];
  
        // Get the HTML elements where data will be displayed
        let heartRateDiv = document.querySelector("#HeartRate");
        let temperatureDiv = document.querySelector("#Temperature");
  
        // Update the content inside circles
        heartRateDiv.innerHTML = `💓 ${lastEntry.field2} BPM`;
        temperatureDiv.innerHTML = `🌡️ ${lastEntry.field1}°C`;
      })
